...
export class EProcessSearchComponent {
	...
	factoryMult(e: Event, i: number) {
		...
		if (item.multLists[s].isChecked) {
			item.multLists[s].isChecked = false
			this.unChecked(e, 'factory', i)
		} else {
			item.multLists[s].isChecked = true
	...
	unChecked(e: Event, s: string, n: number) {
		...
		switch (s) {
			case 'factory':
				...
				count = this.epk_filter.factory.multLists.findIndex(item => item.id === n);
				rObj = this.epk_filter.factory.showMult.findIndex(item => item.id === n);
				break;
			case 'line':
				objItem = this.epk_filter.line
				count = this.epk_filter.line.multLists.findIndex(item => item.id === n);
				rObj = this.epk_filter.line.showMult.findIndex(item => item.id === n);
				break;
			case 'machine':
				objItem = this.epk_filter.machine
				count = this.epk_filter.machine.multLists.findIndex(item => item.id === n);
				rObj = this.epk_filter.machine.showMult.findIndex(item => item.id === n);
				break;
			default:
				break;
		}
