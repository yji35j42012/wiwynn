...
export class EKpiDetailComponent {
	...
	selCheckHandler (obj: any) {
		...
		if (obj.name == 'line_sel') {
			...
			w = this.mofilter_sel;
			if (w.lists[obj.id].name == 'Select All' && !w.lists[obj.id].isChecked) {
				w.showLists = [];
				for (let i = 0; i < w.lists.length; i++) {
					w.lists[i].isChecked = true;
					if (w.lists[i].name !== 'Select All') {
						w.showLists.push(w.lists[i]);
					}
				}
				return;
			} else if (
				w.lists[obj.id].name == 'Select All' &&
				w.lists[obj.id].isChecked
			) {
				w.showLists = [];
				for (let i = 0; i < w.lists.length; i++) {
	...
	getIssChart () {
		if (this.chartBox2) {
			this.chartBox2.destroy();
		}
		this.chartBox2 = new Chart(
			this.el.nativeElement.querySelector('#issueChart'),
			{
				type: 'bar',
				data: {
					labels: [
